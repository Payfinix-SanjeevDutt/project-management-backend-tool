...
# def employeeLogin():
#     return LoginEmployee(request=request).microsoft_login()
...
@auth_blueprint.route("/microsoft-sign-in", methods=["GET"])
def microsoftLogin():
    return LoginEmployee(request=request).microsoft_login()

# @auth_blueprint.route("/callback", methods=["POST"])
# def employeeLogin2():
#     return LoginEmployee(request=request).microsoft_callback()


@auth_blueprint.route("/callback", methods=["GET"])
def microsoftLogin2():
    return LoginEmployee(request).microsoft_callback()
