        ...
        try:
            task_counts = (
                db.session.query(
                    ...
                    func.max(Task.end_date).label("latest_task_end_date") 
                )
            ...
            for emp in task_counts:
                if emp.latest_task_end_date:
                    latest_date = emp.latest_task_end_date.date() 
                    ...
                    idle_days = (today - latest_date).days if today > latest_date else 0
                else:
